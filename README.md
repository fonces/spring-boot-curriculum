...
### 🎯 Phase 8: 総合演習（最終プロジェクト） (6ステップ)

Phase 1〜7で学んだすべての知識を統合し、実践的な「BlogHub」ミニブログアプリケーションを開発します。
REST APIとThymeleafによるWeb画面の両方を実装し、フルスタック開発を体験します。
...
**目標**: REST APIに加えてThymeleafでユーザー向け画面を実装
- Thymeleaf依存関係の追加
- WebController（記事一覧、詳細、投稿フォーム）
- レイアウトテンプレート（共通ヘッダー・フッター）
- 記事一覧画面（ページネーション、タグフィルター）
- 記事詳細画面（コメント表示・投稿フォーム）
- 記事投稿・編集フォーム（バリデーション表示）
- ユーザー登録・ログインフォーム
- セッション管理（Spring Security統合）
- **成果物**: ブラウザで操作可能なブログアプリケーション
