...
> **このステップは総仕上げです！**
> 
> アプリケーションの品質を高め、本番環境で動かせるようにしましょう。
...
重要な機能にテストコードを書いてください。

**テストすべき内容：**
- Mapperのテスト（データベース操作が正しいか）
- Serviceのテスト（ビジネスロジックが正しいか）
- Controllerのテスト（HTTPリクエスト/レスポンスが正しいか）

**考えてほしいこと：**
- `@MybatisTest`でMapperをテストできる
- `@SpringBootTest`で統合テストができる
- `MockMvc`でコントローラーをテストできる
- どんなケースをテストすべきか？（正常系、異常系）

> **💡 ヒント**: `src/test/java`配下にテストコードを配置します

---

### 課題2: Dockerコンテナ化

アプリケーションをDockerで動かせるようにしてください。

**必要なファイル：**
- `Dockerfile` - アプリケーションのコンテナ定義
- `docker-compose.yml` - アプリとデータベースをまとめて起動

**考えてほしいこと：**
- マルチステージビルドで効率化できないか？
- データベースはどうするか？（MySQL/PostgreSQLコンテナ）
- 環境変数で設定を切り替えられるか？

> **💡 ヒント**: マルチステージビルドでイメージサイズを小さくできます
